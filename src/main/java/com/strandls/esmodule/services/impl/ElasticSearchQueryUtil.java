...
/**
 * Elasticsearch Query Utility for ES 9.x
 * Migrated from High Level REST Client to Java API Client
 */
public class ElasticSearchQueryUtil {
	...
	private Query getTermsQuery(MapBoolQuery query) {
		List<FieldValue> values = query.getValues().stream()
	...
	private Query getRangeQuery(MapRangeQuery query) {
		Query rangeQuery = RangeQuery.of(r -> r
			.field(query.getKey())
			.gte(JsonData.of(query.getStart()))
			.lte(JsonData.of(query.getEnd()))
		)._toQuery();
	...
	private Query getMatchPhraseQuery(MapMatchPhraseQuery query) {
		Query matchQuery = MatchPhraseQuery.of(m -> m
			.field(query.getKey())
			.query(query.getValue().toString())
		)._toQuery();
	...
	private void combinationNestedQuery(BoolQuery.Builder masterBoolQuery, BoolQuery.Builder nestedBoolQuery,
		...
		if (StringUtils.isNumeric(nestedPath)) {
			...
			masterBoolQuery.must(NestedQuery.of(n -> n
				.path(path)
	...
	private void buildNestedMatchPhraseOrQuery(List<MapOrMatchPhraseQuery> nestedor, BoolQuery.Builder masterBoolQuery) {
	...
	protected Aggregation getGeoGridAggregation(String field, Integer precision) {
		...
		return GeoHashGridAggregation.of(g -> g
			.field(field)
			.precision(prec)
		)._toAggregation();
	}
	...
	protected Aggregation getTermsAggregation(String field, String subField, Integer size) {
		...
		if (subField != null) {
			builder.aggregations(subField, Aggregation.of(a -> a
				.terms(t -> t.field(subField))
			));
		}
	...
	/**
	 * Replacement for deprecated GeoPolygonQuery - uses GeoShape instead
	 */
	private void applyGeoShapePolygonQuery(List<MapGeoPoint> polygon, BoolQuery.Builder masterBoolQuery,
			String field) throws IOException {

		// Convert polygon points to coordinate list for GeoShape
		...
		// Create GeoShape polygon query
		masterBoolQuery.filter(GeoShapeQuery.of(g -> g
			.field(field)
			.shape(s -> s
				.polygon(p -> p.coordinates(Arrays.asList(coordinates)))
			)
		)._toQuery());
	}
	...
	protected void applyGeoPolygonQuery(List<MapGeoPoint> polygon, BoolQuery.Builder masterBoolQuery,
		...
		masterBoolQuery.should(GeoShapeQuery.of(g -> g
			.field(geoShapeFilterField)
			.shape(s -> s.polygon(p -> p.coordinates(Arrays.asList(coordinates))))
		)._toQuery());
	}
	...
	protected void applyMapBounds(MapBounds bounds, BoolQuery.Builder masterBoolQuery, String geoAggregationField) {
		...
		if (bounds != null) {
			masterBoolQuery.filter(GeoBoundingBoxQuery.of(g -> g
				.field(geoAggregationField)
				.boundingBox(b -> b
					.tlbr(tlbr -> tlbr
						.topLeft(LatLonGeoLocation.of(ll -> ll
							.lat(bounds.getTop())
							.lon(bounds.getLeft())
						))
						.bottomRight(LatLonGeoLocation.of(ll -> ll
